...
export function openFile(path, flags) {
  try {
    return openSync({ path, flag: flags });
  } catch (error) {
...
export function writeFile(filename, data) {
  try {
    ...
    debugLog(3, `writeFile success, data written to '${filename}'`);
  } catch (error) {
    debugLog(1, `writeFile failed for '${filename}':`, error);
  }
}
...
export function readFile(filename) {
  ...
  try {
    ...
    debugLog(2, `Failed to read the file: ${filename}`, error);
    return "";
  }
}
...
export function listDirectory(directory) {
  try {
    return readdirSync({ path: directory });
  } catch (error) {
...
export function dirOrFileExists(path) {
...
let str2ab, ab2str;
try {
  if (typeof hmUtil !== 'undefined' && typeof hmUtil.stringToBuffer === 'function') {
    str2ab = hmUtil.stringToBuffer;
    ab2str = hmUtil.bufferToString;
  } else {
    str2ab = str2ab_fb;
    ab2str = ab2str_fb;
  }
...
export { str2ab_fb, ab2str_fb };

function str2ab_fb(str) {
  const buf = new ArrayBuffer(str.length * 2);
  const view = new Uint16Array(buf);
  const len = str.length;

  let i = 0;
  // batch 4 chars at once
  for (; i < len - 3; i += 4) {
    view[i] = str.charCodeAt(i);
    view[i + 1] = str.charCodeAt(i + 1);
    view[i + 2] = str.charCodeAt(i + 2);
    view[i + 3] = str.charCodeAt(i + 3);
  }
  // remainder
  for (; i < len; i++) {
    view[i] = str.charCodeAt(i);
...
export function writeAsset(filename, data) {
  ...
  const result = writeSync({ fd: file, buffer: buffer });
  ...
  if (file_info) {
    ...
    if (fd !== undefined) {
      ...
      const bytes_read = readSync({
        fd: fd, buffer: file_content_buff, offset: 0,
        length: file_info.size, position: null,
      });
